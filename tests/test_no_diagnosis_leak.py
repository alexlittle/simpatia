...
def test_every_case_file_parses():
    """Every locale file on disk must parse into PatientCase."""
    from simpatia.config import settings

    paths = sorted((settings.content_dir / "cases").glob("*/[a-z][a-z]-*.yaml"))
    assert paths, "No case files found — check settings.content_dir"

    for path in paths:
        raw = yaml.safe_load(path.read_text(encoding="utf-8"))
        assert raw is not None, f"{path.relative_to(settings.content_dir)} is empty"
        try:
            PatientCase.model_validate(raw)
        except ValidationError as e:
            pytest.fail(f"{path.relative_to(settings.content_dir)}:\n{e}")
