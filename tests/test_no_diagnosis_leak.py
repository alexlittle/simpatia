...
def test_no_case_yaml_contains_a_diagnosis_key(tmp_path):
    ...
    for path in (get_settings().content_dir / "cases").glob("*/[a-z][a-z]-*.yaml"):
...
def _case_files():
    ...
    return sorted((get_settings().content_dir / "cases").glob("*/[a-z][a-z]-*.yaml"))
...
def test_declared_locales_exist():
    ...
    for meta_path in (get_settings().content_dir / "cases").glob("*/meta.yaml"):
